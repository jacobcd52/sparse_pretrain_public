#!/usr/bin/env python3
"""
Regenerate Pareto comparison plots for frozen layer norm results.

This script creates comparison plots for all 4 models in both zero and mean
...
def create_comparison_plot(base_dir: Path, ablation_type: str, output_name: str):
    """Create a comparison plot with proper formatting."""
    
    print(f"\n{'='*60}")
    print(f"Creating {ablation_type} ablation comparison plot (FROZEN LN)...")
    print(f"{'='*60}")
    ...
    for model_short, label, color in models:
        # Note: frozen LN dirs have the suffix _frozenln
        model_dir_name = f"{model_short}_{ablation_type}_noembed_frozenln"
    ...
    ax.set_title(f"Pronoun Task (Frozen LN): Pareto Curves ({ablation_type.upper()} Ablation)", fontsize=16)
    ax.set_yscale('log')
...
def create_combined_plot(base_dir: Path):
    ...
    print(f"\n{'='*60}")
    print("Creating combined (zero vs mean) plot (FROZEN LN)...")
    print(f"{'='*60}")
    ...
    for (model_short, label), color in zip(models, colors):
        for ablation_suffix, linestyle, marker in [("zero", "-", "o"), ("mean", "--", "s")]:
            # Note: frozen LN dirs have the suffix _frozenln
            model_dir_name = f"{model_short}_{ablation_suffix}_noembed_frozenln"
            model_dir = base_dir / model_dir_name
    ...
    ax.set_xlabel("Circuit Size (nodes)", fontsize=14)
    ax.set_ylabel("Superval Loss", fontsize=14)
    ax.set_title("Pronoun Task (Frozen LN): Zero vs Mean Ablation", fontsize=16)
    ax.set_yscale('log')
    ...
    plot_path = base_dir / "pareto_comparison_zero_vs_mean_frozenln.png"
    plt.savefig(plot_path, dpi=150)
    plt.close()
    print(f"\nSaved: {plot_path}")
...
def main():
    ...
    if not base_dir.exists():
        ...
        print("Run the CARBS sweeps first with run_carbs_frozenln_all_models.py")
        return
