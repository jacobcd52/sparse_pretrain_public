...
NEW HYPERPARAMETERS:
- k_coef_center = 1e-3 (was 0.0002)
- lr_center = 1e-2

Results saved to carbs_results_pronoun_iswhen_new_hparams/
"""
...
def main():
    ...
    output_base_dir = f"my_sparse_pretrain/outputs/carbs_results_pronoun_{task_suffix}_new_hparams"
    
    print("=" * 70)
    print(f"CARBS SWEEP: ss_bridges_d1024_f0.015625")
    print(f"Task: {task_name}")
    print(f"  Male names → 'is', Female names → 'when'")
    print(f"NEW HPARAMS: k_coef_center=1e-3, lr_center=1e-2")
    ...
    config = CleanSweepConfig(
        ...
        k_coef_center=1e-3,     # NEW: was 0.0002
        lr_center=1e-2,         # NEW: explicitly set
    )
