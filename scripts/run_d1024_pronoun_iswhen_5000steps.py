...
Same hparams as iswhen_new_hparams but with 5000 steps.

Results saved to carbs_results_pronoun_iswhen_5000steps/
"""
...
def main():
    ...
    output_base_dir = f"my_sparse_pretrain/outputs/carbs_results_pronoun_{task_suffix}_5000steps"
    
    print("=" * 70)
    print(f"CARBS SWEEP: ss_bridges_d1024_f0.015625")
    print(f"Task: {task_name}")
    print(f"  Male names → 'is', Female names → 'when'")
    print(f"HPARAMS: k_coef_center=1e-3, lr_center=1e-2, num_steps=5000")
    ...
    # Create config - same as new_hparams but with 5000 steps
    config = CleanSweepConfig(
        ...
        num_steps=5000,  # Changed from 2000
        batch_size=64,
        task_max_length=0,
