#!/usr/bin/env python3
"""
Run 100 seeds for IOI Relaxed task, filter by target loss, then evaluate
same-gender vs opposite-gender performance.
"""
...
from my_sparse_pretrain.src.pruning.tasks import get_task, IOIRelaxedTask
...
class IOIRelaxedSameGenderTask(IOIRelaxedTask):
    ...
    def generate_example(self):
        from my_sparse_pretrain.src.pruning.tasks import TaskExample
        
        # Both names same gender
...
def evaluate_gender_performance(
    masked_model,
    tokenizer,
    config: Dict[str, Any],
    device: str,
    ...
    num_batches = 20  # Enough samples for good statistics
    ...
    for task_name, task in [("opposite_gender", opposite_gender_task), ("same_gender", same_gender_task)]:
        ...
        for _ in range(num_batches):
            ...
            with torch.no_grad(), autocast_ctx:
                # Forward pass
                outputs = masked_model.forward_with_mask(positive_ids)
                logits = outputs.logits  # [batch, seq, vocab]
                
                # Get logits at eval positions
                batch_indices = torch.arange(batch_size, device=device)
                eval_logits = logits[batch_indices, eval_positions, :]  # [batch, vocab]
                
                # Get probabilities for correct and incorrect tokens
                probs = torch.softmax(eval_logits, dim=-1)
                correct_probs = probs[batch_indices, correct_tokens]  # [batch]
                incorrect_probs = probs[batch_indices, incorrect_tokens]  # [batch]
                
                # Compute P(correct) / (P(correct) + P(incorrect))
...
def main():
    # Configuration
    CHECKPOINT_DIR = Path("my_sparse_pretrain/outputs/carbs_results_ioi_relaxed/ss_bridges_d1024_f0.015625_zero_noembed")
    NUM_SEEDS = 100
    DEVICE = "cuda"
    ...
    print("=" * 70)
    print("Running 100 Seeds for IOI Relaxed")
    print("=" * 70)
    ...
    # Phase 2: Evaluate gender performance for successful seeds
    print(f"\nPhase 2: Evaluating same-gender vs opposite-gender performance...")
    
    # Create pruning config for evaluation
    pruning_config = PruningConfig(
        batch_size=config["batch_size"],
        seq_length=config.get("task_max_length", 0),
        device=DEVICE,
