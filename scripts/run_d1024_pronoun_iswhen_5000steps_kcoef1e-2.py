#!/usr/bin/env python
"""
Run CARBS sweep + evals + HTML for d1024 model on the "iswhen" task.

Target depends on name gender:
- Male names → "is"
- Female names → "when"

Same as iswhen_5000steps but with k_coef_center=1e-2.

Results saved to carbs_results_pronoun_iswhen_5000steps_kcoef1e-2/
"""
...
def main():
    ...
    output_base_dir = f"my_sparse_pretrain/outputs/carbs_results_pronoun_{task_suffix}_5000steps_kcoef1e-2"
    
    print("=" * 70)
    print(f"CARBS SWEEP: ss_bridges_d1024_f0.015625")
    print(f"Task: {task_name}")
    print(f"  Male names → 'is', Female names → 'when'")
    print(f"HPARAMS: k_coef_center=1e-2, lr_center=1e-2, num_steps=5000")
    ...
    # Create config with k_coef_center=1e-2
    config = CleanSweepConfig(
        ...
        k_coef_center=1e-2,  # Changed from 1e-3
        lr_center=1e-2,
    )
